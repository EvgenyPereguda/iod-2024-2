...
function showCreateNewLikeFormDialog(postId){
  ...
  console.log("submitCreateNewLikeForm(" +`'${postId}'`+")");
...
async function loadUsers(){
  ...
  .then((json) => {
    json.data.forEach((item) => {   
      placeUserItem(item.firstName, item.lastName, item.id);
    });
  })
  .catch((response) => {      
...
async function loadPosts(id = 0){
  ...
  .then((json) => {
    json.data.forEach((item) => { 
      placePostItem("allPostsList", item.id, item.Title, item.Description, item.Image, item.user.firstName, item.user.lastName);
    });
  })
  .catch((response) => {      
    alert(`Response status = ${response.status}, message ${response.statusText}`);
  ...
  if(id != 0)
  {
    ...
    .then((json) => {
      json.data.forEach((item) => { 
        placePostItem("postsList", item.id, item.Title, item.Description, item.Image, item.user.firstName, item.user.lastName);
      });
    })
    .catch((response) => {      
      alert(`Response status = ${response.status}, message ${response.statusText}`);
  ...
  if(id != 0)
    {
      ...
      .then((json) => {
        json.data.forEach((item) => { 
          placeLikeItem(target, item.Comment, item.user.firstName, item.user.lastName);
        });
      })
      .catch((response) => {      
        alert(`Response status = ${response.status}, message ${response.statusText}`);
