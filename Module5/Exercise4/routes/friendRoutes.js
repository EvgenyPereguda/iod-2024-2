...
router.get("/filter", (req, res) => {
  ...
  else if(letter)
  {
    ...
    for (let friend of friends) {
      if (friend.name.startsWith(letter)) selectedFriends.push(friend);
    }
  
    if (selectedFriends.length > 0)
      res.status(200).json({ result: selectedFriends });
    else res.status(404).json({ result: `Friends not found` });
  }
...
router.get("/info", (req, res) => {
  ...
  linfo["user-agent"] = req.headers["user-agent"];
  linfo["content-type"] = req.headers["content-type"];
  linfo["accept"] = req.headers["accept"];
  ...
  let friend = friends.find((friend) => friend.id == friendId);
    ...
    if(friend)
    {
      try{
        ...
        res.status(404).json({ result: `Friend with ID ${friendId} cannot be updated by error: ` + err.message });  
      }
