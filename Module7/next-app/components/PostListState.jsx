...
export default function PostListState() {
  ...
  const [postsResult, setDispatch] = useState({
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        setDispatch(reducer(postsResult, { type: "FETCH_SUCCESS", payload: response.data }));

        })
      .catch((error) => {

        setDispatch(reducer(postsResult, { type: "FETCH_ERROR", payload: error.message}));
